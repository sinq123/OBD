...
bool CDynDlg::UpConstLoad(void)
{
	...
	bool bDieselOil(false);
	if (L"3" == str)
	{
		bDieselOil = true; 
	}

	CString strStartTime, strEnd;
	strEnd.Format(L"%s",  COleDateTime::GetCurrentTime().Format(L"%Y-%m-%d %H-%M-%S"));

	COleDateTime otd1, otd2;
	strStartTime = si.GetString(L"LowerRangeLoadConst", L"StartTime", COleDateTime::GetCurrentTime().Format(L"%Y-%m-%d %H:%M:%S"));
	if (!otd1.ParseDateTime(strStartTime))
	{
		otd1 = COleDateTime::GetCurrentTime();
	}
	strStartTime = si.GetString(L"HigherRangeLoadConst", L"StartTime", COleDateTime::GetCurrentTime().Format(L"%Y-%m-%d %H:%M:%S"));
	if (!otd2.ParseDateTime(strStartTime))
	{
		otd2 = COleDateTime::GetCurrentTime();
	}
	if (otd1 > otd2)
	{
		strStartTime.Format(L"%s",otd2.Format(L"%Y-%m-%d %H-%M-%S"));
	}
	else
	{
		strStartTime.Format(L"%s",otd1.Format(L"%Y-%m-%d %H-%M-%S"));
	}

	CString strAvitualTime, strArealTime, strBvitualTime, strBrealTime, strCvitualTime, strCrealTime, strDvitualTime,
		strDrealTime, strALSpower, strBLSpower, strCLSpower, strDLSpower, strcheckResult, strremark;

	//registCode	启动授权码	字符串	50	启动时环保软件传递的参数授权码，只本次启动有效
	//AvitualTime	A速加载滑行理论时间	数字	5.1	单位：s,区间：（80-60）km/h 柴油车
	strAvitualTime = si.GetString(L"HigherRangeLoadConst", L"CCDT", L"0.0");
	//ArealTime	A速加载滑行实际时间	数字	5.1	单位：s,区间：(80-60）km/h//柴油车
	strArealTime = si.GetString(L"HigherRangeLoadConst", L"ACDT", L"0.0");
	//BvitualTime	B速加载滑行理论时间	数字	5.1	单位：s,区间：(65-45）km/h//柴油车
	strBvitualTime = si.GetString(L"LowerRangeLoadConst", L"CCDT", L"0.0");
	//BrealTime	B速加载滑行实际时间	数字	5.1	单位：s,区间：(65-45）km/h//柴油车
	strBrealTime = si.GetString(L"LowerRangeLoadConst", L"ACDT", L"0.0");
	//CvitualTime	C速加载滑行理论时间	数字	5.1	单位：s,区间：（50-30）km/h//柴油车和汽油车
	strCvitualTime = si.GetString(L"LowerRangeLoadConst", L"GZLugdownRangeCCDT0", L"0.0");
	//CrealTime	C速加载滑行实际时间	数字	5.1	单位：s,区间：(50-30）km/h//柴油车和汽油车
	strCrealTime = si.GetString(L"LowerRangeLoadConst", L"GZLugdownRangeACDT0", L"0.0");
	//DvitualTime	D速加载滑行理论时间	数字	5.1	单位：s,区间：(35-15）km/h//柴油车和汽油车
	strDvitualTime = si.GetString(L"LowerRangeLoadConst", L"GZLugdownRangeCCDT1", L"0.0");
	//DrealTime	D速加载滑行实际时间	数字	5.1	单位：s,区间：(35-15）km/h//柴油车和汽油车
	strDrealTime = si.GetString(L"LowerRangeLoadConst", L"GZLugdownRangeACDT1", L"0.0");
	//ALSpower	A速加载滑行功率	数字	3.1	单位：kW，柴油车
	strALSpower = si.GetString(L"HigherRangeLoadConst", L"PowerStandardValue", L"10");
	//BLSpower	B速加载滑行功率	数字	3.1	单位：kW，柴油车
	strBLSpower = si.GetString(L"LowerRangeLoadConst", L"PowerStandardValue", L"8");
	//CLSpower	C速加载滑行功率	数字	3.1	单位：kW，柴油车和汽油车
	strCLSpower = si.GetString(L"LowerRangeLoadConst", L"PowerStandardValue", L"8");
	//DLSpower	D速加载滑行功率	数字	3.1	单位：kW，柴油车和汽油车
	strDLSpower = si.GetString(L"LowerRangeLoadConst", L"PowerStandardValue", L"8");
	//checkResult	检查结果	字符串	1	1表示通过，0表示未通过
	CString strTemp;
	strTemp = si.GetString(L"HigherRangeLoadConst", L"Pass", L"1");
	strcheckResult = si.GetString(L"LowerRangeLoadConst", L"Pass", L"1");
	if (strTemp == L"1"
		&& strcheckResult == L"1"
		)
	{
	...
	//checkTimeStart	检查开始时间	字符串	19	yyyy-MM-dd HH:mm:ss
	//checkTimeEnd	检查结束时间	字符串	19	yyyy-MM-dd HH:mm:ss
	//remark	工控软件版本号	字符串	100	

	std::wstring strRet;
	int nRet(99);
	if (bDieselOil)
	{
		nRet = CGZWebServic_API::cgjLSPSelfcheck(theApp.m_pchURL, bDieselOil, strAvitualTime.GetString(), strArealTime.GetString(),
			strBvitualTime.GetString(), strBrealTime.GetString(), strCvitualTime.GetString(), strCrealTime.GetString(),
