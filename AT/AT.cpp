...
void CATApp::GetConfig(void)
{
	...
	if (0x00 == CNHCommonAPI::GetFilePathEx(L"Config", L"WebServiceLib.ini", wchPath))
	{
		CSimpleIni si(wchPath);

		m_strURL = si.GetString(L"EpbServer", L"WebServer", L"");
		if (NULL != m_pchURL)
		{
			free(m_pchURL);
			m_pchURL = NULL;
		...
		m_strStationNum = si.GetString(L"EpbServer", L"StationNo", L"");
		m_strLineNum = si.GetString(L"EpbServer", L"LineNo", L"");
	}
