...
void CNHSystemCheckDlg::OnBnClickedBtnSystemChechSelf()
{
	...
	)
	{
		...
		SHBMsg sHBMsg;
		CZYHttp_PAI::GetInstance().SetLeakCheck(L"H02", theApp.m_sUserInfo.wchName,L"123456200001010101", sHBMsg);
	}
	...
	)
	{
		...
		SHBMsg sHBMsg;
		CZYHttp_PAI::GetInstance().SetLeakCheck(L"H03", theApp.m_sUserInfo.wchName,L"123456200001010101", sHBMsg);
	}
...
void CNHSystemCheckDlg::OnBnClickedBtnLeakUpload()
{
	// TODO: 在此添加控件通知处理程序代码
	SHBMsg sHBMsg;
	if (!CZYHttp_PAI::GetInstance().SetLeakCheck(L"H03", theApp.m_sUserInfo.wchName,L"123456200001010101", sHBMsg))
	{
		CString strMsg;
		strMsg.Format(L"上传失败\r\n %s\r\n%s",sHBMsg.code.c_str(), sHBMsg.msg.c_str());
		MessageBox(strMsg, L"上传", MB_ICONEXCLAMATION);
	}
