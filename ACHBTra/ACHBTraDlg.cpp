...
int CACHBTraDlg::m_nWebServicePort(10022);
bool CACHBTraDlg::m_bWebServiceRun(false);
int CACHBTraDlg::m_nHead(0);
int CACHBTraDlg::m_nTail(0);

struct soap* CACHBTraDlg::m_mapSoap;
SOAP_SOCKET  CACHBTraDlg::m_Queue_Soap[MAX_QUEUE];
pthread_mutex_t  CACHBTraDlg::m_Queue_CS;
pthread_cond_t  CACHBTraDlg::m_Queue_CV;

CACHBTraDlg* g_pCACHBTraDlg = NULL;


CACHBTraDlg::CACHBTraDlg(CWnd* pParent /*=NULL*/)
	: CDialogEx(CACHBTraDlg::IDD, pParent)
{
	m_hIcon = AfxGetApp()->LoadIcon(IDR_MAINFRAME);
}

CACHBTraDlg::~CACHBTraDlg()
{
}

void CACHBTraDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_COM_DET_TYPE, m_cbDelType);
}

BEGIN_MESSAGE_MAP(CACHBTraDlg, CDialogEx)
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_BTN_START_WEBSERVICE, &CACHBTraDlg::OnBnClickedBtnStartWebservice)
	ON_WM_CLOSE()
	ON_WM_TIMER()
END_MESSAGE_MAP()
...
//显示。
HCURSOR CACHBTraDlg::OnQueryDragIcon()
{
	return static_cast<HCURSOR>(m_hIcon);
}

void CACHBTraDlg::OnBnClickedBtnStartWebservice()
{
	// TODO: 在此添加控件通知处理程序代码
	CString str;
...
void CACHBTraDlg::OnTimer(UINT_PTR nIDEvent)
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
...
void CACHBTraDlg::InitCtrls(void)
{
	CStringW str;
...
void CACHBTraDlg::LoadConfig(void)
{
	wchar_t wchFilePath[MAX_PATH] = { L'\0' };
	CNHCommonAPI::GetHLDFilePath(L"Config", L"HBWebServerTra.ini", wchFilePath);
...
UINT CACHBTraDlg::GSoapServerThread(LPVOID pParam)
{
	struct soap sSoap;
...
void CACHBTraDlg::StartWebService(void)
{
	AfxBeginThread(GSoapServerThread, this);
}

void CACHBTraDlg::StopWebService(void)
{
...
int CACHBTraDlg::HttpGet(soap* soap)
{
	// 获取wsdl文件
	wchar_t wchFilePath[MAX_PATH] = { L'\0' };
...
void *  CACHBTraDlg::Process_Queue(void *soap) 
{ 
...
int  CACHBTraDlg::Enqueue(SOAP_SOCKET sock) 
{ 
	int nStatus = SOAP_OK; 
	int nNext; 
...
SOAP_SOCKET  CACHBTraDlg::Dequeue() 
{ 
	SOAP_SOCKET sock; 
	pthread_mutex_lock(&m_Queue_CS); 
...
std::wstring CACHBTraDlg::LogFilePath(void)
{
	// 日志文件所在文件夹路径
...
std::wstring CACHBTraDlg::ModXml(const std::wstring* strUTF8XmlDoc)
{
	// 中文处理成URL
	//CURLCode::EncodeW();
	// URL处理成中文
...
bool CACHBTraDlg::GetSimulationData(CString& strJQMKD, CString& strJSFHZ, CString& strQYCGQXH, CString& strCS, 
...
int __ns1__query(struct soap*, _ns1__query *ns1__query, _ns1__queryResponse &ns1__queryResponse)
{
	...
	strXmlDoc2 = g_pCACHBTraDlg->ModXml(ns1__query->xmlDoc);
	ns1__query->xmlDoc = & strXmlDoc2;

	if (g_pCACHBTraDlg->m_bIsLog)
	{
		CString strLog;
		strLog.Format(L"%s->", ns1__query->xmlDoc->c_str());

		CNHLogAPI::WriteLogEx(g_pCACHBTraDlg->LogFilePath().c_str(), L"接收", L"__ns1__query", strLog);
	}

	int nRet = soap_call___ns1__query(&soapClient, g_pCACHBTraDlg->m_chGAURL, NULL, ns1__query, ns1__queryResponse);
...
int __ns1__write(struct soap*, _ns1__write *ns1__write, _ns1__writeResponse &ns1__writeResponse)
{
	struct soap soapClient;
	soap_init(&soapClient);
	soap_set_mode(&soapClient, SOAP_C_UTFSTRING);
	...
	std::wstring strXmlDoc2;
	strXmlDoc2 = g_pCACHBTraDlg->ModXml(ns1__write->xmlDoc);
	ns1__write->xmlDoc = & strXmlDoc2;

	if (g_pCACHBTraDlg->m_bIsLog)
	{
		CString strLog;
		strLog.Format(L"%s->", ns1__write->xmlDoc->c_str());

		CNHLogAPI::WriteLogEx(g_pCACHBTraDlg->LogFilePath().c_str(), L"接收", L"__ns1__write", strLog);
	}
	int nRet = soap_call___ns1__write(&soapClient, g_pCACHBTraDlg->m_chGAURL, NULL, ns1__write, ns1__writeResponse);
...
int __ns1__query_(struct soap*, _ns1__query *ns1__query, _ns1__queryResponse &ns1__queryResponse)
{
	struct soap soapClient;
	soap_init(&soapClient);
	soap_set_mode(&soapClient, SOAP_C_UTFSTRING);
	...
	if (g_pCACHBTraDlg->m_bIsLog)
	{
		...
		CNHLogAPI::WriteLogEx(g_pCACHBTraDlg->LogFilePath().c_str(), L"接收", L"__ns1__query_", strLog);
	}
	int nRet = soap_call___ns1__query_(&soapClient, g_pCACHBTraDlg->m_chGAURL, NULL, ns1__query, ns1__queryResponse);
...
int __ns1__write_(struct soap*, _ns1__write *ns1__write, _ns1__writeResponse &ns1__writeResponse)
{
	struct soap soapClient;
	soap_init(&soapClient);
	soap_set_mode(&soapClient, SOAP_C_UTFSTRING);
	...
	if (g_pCACHBTraDlg->m_bIsLog)
	{
		...
		CNHLogAPI::WriteLogEx(g_pCACHBTraDlg->LogFilePath().c_str(), L"接收", L"__ns1__write_", strLog);
	}
	int nRet = soap_call___ns1__write_(&soapClient, g_pCACHBTraDlg->m_chGAURL, NULL, ns1__write, ns1__writeResponse);
...
int __ns1__query__(struct soap*, _ns1__query *ns1__query, _ns1__queryResponse &ns1__queryResponse)
{
	struct soap soapClient;
	soap_init(&soapClient);
	soap_set_mode(&soapClient, SOAP_C_UTFSTRING);
	...
	if (g_pCACHBTraDlg->m_bIsLog)
	{
		...
		CNHLogAPI::WriteLogEx(g_pCACHBTraDlg->LogFilePath().c_str(), L"接收", L"__ns1__query__", strLog);
	}
	int nRet = soap_call___ns1__query__(&soapClient, g_pCACHBTraDlg->m_chGAURL, NULL, ns1__query, ns1__queryResponse);
...
int __ns1__write__(struct soap*, _ns1__write *ns1__write, _ns1__writeResponse &ns1__writeResponse)
{
	struct soap soapClient;
	soap_init(&soapClient);
	soap_set_mode(&soapClient, SOAP_C_UTFSTRING);
	...
	if (g_pCACHBTraDlg->m_bIsLog)
	{
		...
		CNHLogAPI::WriteLogEx(g_pCACHBTraDlg->LogFilePath().c_str(), L"接收", L"__ns1__write__", strLog);
	}
	int nRet = soap_call___ns1__write__(&soapClient, g_pCACHBTraDlg->m_chGAURL, NULL, ns1__write, ns1__writeResponse);
...
int __ns1__query___(struct soap*, _ns1__query *ns1__query, _ns1__queryResponse &ns1__queryResponse)
{
	struct soap soapClient;
	soap_init(&soapClient);
	soap_set_mode(&soapClient, SOAP_C_UTFSTRING);
	...
	if (g_pCACHBTraDlg->m_bIsLog)
	{
		...
		CNHLogAPI::WriteLogEx(g_pCACHBTraDlg->LogFilePath().c_str(), L"接收", L"__ns1__query___", strLog);
	}
	int nRet = soap_call___ns1__query___(&soapClient, g_pCACHBTraDlg->m_chGAURL, NULL, ns1__query, ns1__queryResponse);
...
int __ns1__write___(struct soap*, _ns1__write *ns1__write, _ns1__writeResponse &ns1__writeResponse)
{
	struct soap soapClient;
	soap_init(&soapClient);
	soap_set_mode(&soapClient, SOAP_C_UTFSTRING);
	...
	if (g_pCACHBTraDlg->m_bIsLog)
	{
		...
		CNHLogAPI::WriteLogEx(g_pCACHBTraDlg->LogFilePath().c_str(), L"接收", L"__ns1__write___", strLog);
	}
	int nRet = soap_call___ns1__write___(&soapClient, g_pCACHBTraDlg->m_chGAURL, NULL, ns1__write, ns1__writeResponse);
