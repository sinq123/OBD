...
/*static*/ const UINT CStdioFileExAAndUn::modeWriteUnicode = 0x20000; // Add this flag to write in Unicode

CStdioFileExAAndUn::CStdioFileExAAndUn(): CStdioFile()
{
	m_bIsUnicodeText = false;
}

CStdioFileExAAndUn::CStdioFileExAAndUn(LPCTSTR lpszFileName,UINT nOpenFlags)
	:CStdioFile(lpszFileName, ProcessFlags(lpszFileName, nOpenFlags))
{
}

BOOL CStdioFileExAAndUn::Open(LPCTSTR lpszFileName,UINT nOpenFlags,CFileException* pError /*=NULL*/)
{
...
BOOL CStdioFileExAAndUn::ReadString(CString& rString)
{
	...
	if (bReadData)
	{
		...
		if (nLen > 1 && rString.Mid(nLen-2) == sNEWLINE_)
		{
...
// --------------------------------------------------------------------------------------------
//
// CStdioFileExAAndUn::WriteString()
//
// --------------------------------------------------------------------------------------------
// Returns:    void
...
//     CStdioFileExAAndUn::modeWriteUnicode flag. Override of base class function.
...
// Exceptions: None.
//
void CStdioFileExAAndUn::WriteString(LPCTSTR lpsz)
{
	// If writing Unicode and at the start of the file, need to write byte mark
	if (m_nFlags & CStdioFileExAAndUn::modeWriteUnicode)
	{
...
UINT CStdioFileExAAndUn::ProcessFlags(const CString& sFilePath, UINT& nOpenFlags)
{
	m_bIsUnicodeText = false;

	// If we have writeUnicode we must have write or writeRead as well
#ifdef _DEBUG
	if (nOpenFlags & CStdioFileExAAndUn::modeWriteUnicode)
	{
...
// --------------------------------------------------------------------------------------------
//
// CStdioFileExAAndUn::IsFileUnicode()
//
// --------------------------------------------------------------------------------------------
// Returns:    bool
...
// Exceptions: None.
//
/*static*/ bool CStdioFileExAAndUn::IsFileUnicode(const CString& sFilePath)
{
	CFile    file;
...
unsigned long CStdioFileExAAndUn::GetCharCount()
{
	int    nCharSize;
...
// --------------------------------------------------------------------------------------------
//
// CStdioFileExAAndUn::GetUnicodeStringFromMultiByteString()
//
// --------------------------------------------------------------------------------------------
// Returns:    bool
...
// Exceptions: None.
//
bool CStdioFileExAAndUn::GetUnicodeStringFromMultiByteString(char * szMultiByteString, wchar_t* szUnicodeString, short nUnicodeBufferSize, UINT nCodePage)
{
	bool  bOK = true;
	int  nReturn = 0;
	CString sErrorMsg;
...
// --------------------------------------------------------------------------------------------
//
// CStdioFileExAAndUn::GetMultiByteStringFromUnicodeString()
//
// --------------------------------------------------------------------------------------------
// Returns:    BOOL
...
// Exceptions: None.
//
BOOL CStdioFileExAAndUn::GetMultiByteStringFromUnicodeString(wchar_t * szUnicodeString, char* szMultiByteString,
