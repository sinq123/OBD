...
: m_socket(INVALID_SOCKET)
{
	...
	strFileName.Format(L"%s_联网.log", odtNow.Format(L"%Y-%m-%d"));
...
bool CYJSocket::Open(LPCTSTR szIP/*=NULL*/, const int nPort/*=NULL*/)
{
	// 关闭已存在的串口句柄
	Close();
	Sleep(500);
	...
	{
		CNHLogAPI::WriteLogEx(m_strLogFilePath, L"CYJSocket::Open", L"连接成功", strLog);
		return true;
	}
	else
	{
		CNHLogAPI::WriteLogEx(m_strLogFilePath, L"CYJSocket::Open", L"连接失败", strLog);
		Close();
		return false;
	}
}
...
bool CYJSocket::Close(void)
{
	if (m_socket != INVALID_SOCKET)
	{
		...
		CNHLogAPI::WriteLogEx(m_strLogFilePath, L"CYJSocket::Close", L"关闭连接", L"");
	}
	return true;
}

bool CYJSocket::IsOpen(void) const
{
	return m_socket != INVALID_SOCKET;
}

bool CYJSocket::SendAndRecvPacket(LPCTSTR szSend, std::wstring &strRecv)
{
	if (!IsOpen())
	{
		return false;
	...
	{
		CNHLogAPI::WriteLogEx(m_strLogFilePath, L"CYJSocket::SendAndRecvPacket", L"发送成功", szSend);
		...
		{
			CNHLogAPI::WriteLogEx(m_strLogFilePath, L"CYJSocket::SendAndRecvPacket", L"接收失败", L"NULL");				
			return false;
		}
		else
		{
			...
			{
				CNHLogAPI::WriteLogEx(m_strLogFilePath, L"CYJSocket::SendAndRecvPacket", L"接收成功", strData.c_str());				
				return true;
			}
			else
			{
				CNHLogAPI::WriteLogEx(m_strLogFilePath, L"CYJSocket::SendAndRecvPacket", L"接收失败", strData.c_str());				
				return false;
			}
		}
	}
	...
	{
		CNHLogAPI::WriteLogEx(m_strLogFilePath, L"CYJSocket::SendAndRecvPacket", L"发送失败", szSend);
	}
	return false;
}

bool CYJSocket::RecvPacket(std::wstring &strRecv)
{
	if (!IsOpen())
	{
		return false;
...
bool CYJSocket::RecvAll(SOCKET sockfd, char *recvbuf, int len)
{
	// 循环接收
	int nLeft = len;
	int nIndex = 0;
	...
	{  
		...
		{
			CNHLogAPI::WriteLogEx(m_strLogFilePath, L"CYJSocket::RecvAll", L"接收超时", L"");
			return false;
		}
		else if (nRet == 0)
		{
			CNHLogAPI::WriteLogEx(m_strLogFilePath, L"CYJSocket::RecvAll", L"服务端已断开", L"");
			Close();
			return false;
		}
		nLeft -= nRet;
...
bool CYJSocket::SendAll(SOCKET sockfd, char *sendbuf, int len)
{
	//CString strTemp;
	//for (int i=0; i<len; i++)
	//{
	...
	while (nLeft > 0)
	{
		...
		if (nRet == SOCKET_ERROR)
		{
			CNHLogAPI::WriteLogEx(m_strLogFilePath, L"CYJSocket::SendAll", L"服务端已断开", L"");
			Close();
			return false;
		}
		nLeft -= nRet;
