...
IMPLEMENT_DYNAMIC(CNHA503XGasCheckDlg2, CDialogZoom)

CNHA503XGasCheckDlg2::CNHA503XGasCheckDlg2(CWnd* pParent /*=NULL*/)
	: CDialogZoom(CNHA503XGasCheckDlg2::IDD, pParent)
	, m_nStep(0)
...
CNHA503XGasCheckDlg2::~CNHA503XGasCheckDlg2()
{
...
void CNHA503XGasCheckDlg2::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_MATHLABLE8, m_mlb12);
	DDX_Control(pDX, IDC_MATHLABLE13, m_mlb7);
	DDX_Control(pDX, IDC_MATHLABLE14, m_mlb8);
	DDX_Control(pDX, IDC_MATHLABLE15, m_mlb9);
	DDX_Control(pDX, IDC_MATHLABLE16, m_mlb10);
	DDX_Control(pDX, IDC_MATHLABLE17, m_mlb11);
...
BEGIN_MESSAGE_MAP(CNHA503XGasCheckDlg2, CDialogZoom)
	ON_BN_CLICKED(IDC_BUTTON_QUIT, &CNHA503XGasCheckDlg2::OnBnClickedButtonQuit)
	ON_BN_CLICKED(IDC_BUTTON_START, &CNHA503XGasCheckDlg2::OnBnClickedButtonStart)
	ON_BN_CLICKED(IDC_BUTTON_STOP, &CNHA503XGasCheckDlg2::OnBnClickedButtonStop)
	ON_WM_TIMER()
	ON_STN_CLICKED(IDC_STATIC_NO_RE, &CNHA503XGasCheckDlg2::OnStnClickedStaticNoRe)
END_MESSAGE_MAP()


// CNHA503XGasCheckDlg 消息处理程序

BOOL CNHA503XGasCheckDlg2::OnInitDialog()
{
...
void CNHA503XGasCheckDlg2::InitCtrls(void)
{
	...
	m_mlb10.SetScriptText(L"CO_2[%]");
	m_mlb11.SetScriptText(L"O_2[%]");
	m_mlb12.SetScriptText(L"CO_2通道");
...
void CNHA503XGasCheckDlg2::ImportGasCheckLimit(void)
{
	CString strPath;
	// 获取exe(dll)文件绝对路径
	...
	strPath = strPath.Left(strPath.ReverseFind(_T('\\')));
	// 配置文件路径
	strPath = strPath + L"\\App_Data\\LimitOfAnaGasCheck.ini";
	// 去除配置文件只读属性
	...
	CString str;
	// HC绝对误差限值
	str = si.GetString(L"LimitOfAnaGasCheck", L"HCAELimit", L"12");
	ASSERT(L"" != str);
	m_nHCAELimit = _wtoi(str);
	if (m_nHCAELimit < 0)
	{
		m_nHCAELimit = abs(m_nHCAELimit);
	}

	// CO绝对误差限值
	str = si.GetString(L"LimitOfAnaGasCheck", L"COAELimit", L"0.06");
	ASSERT(L"" != str);
	...
	// 放大100倍,并四舍五入
	m_nCOAELimit = (int)(f * AMPLIFICATION_FACTOR + 0.5f);

	// CO2绝对误差限值
	str = si.GetString(L"LimitOfAnaGasCheck", L"CO2AELimit", L"0.5");
	ASSERT(L"" != str);
	f = (float)_wtof(str);
	if (f < -10e-6)
	{
	...
	// 放大100倍,并四舍五入
	m_nCO2AELimit = (int)(f * AMPLIFICATION_FACTOR + 0.5f);

	// NO绝对误差限值
	str = si.GetString(L"LimitOfAnaGasCheck", L"NOAELimit", L"25");
	ASSERT(L"" != str);
	m_nNOAELimit = _wtoi(str);
	if (m_nNOAELimit < 0)
	{
		m_nNOAELimit = abs(m_nNOAELimit);
	}

	// HC相对误差限值
	str = si.GetString(L"LimitOfAnaGasCheck", L"HCRELimit", L"5");
	ASSERT(L"" != str);
	m_nHCRELimit = _wtoi(str);
	if (m_nHCRELimit < 0)
	{
		m_nHCRELimit = abs(m_nHCRELimit);
	}

	// CO相对误差限值
	str = si.GetString(L"LimitOfAnaGasCheck", L"CORELimit", L"5");
	ASSERT(L"" != str);
	m_nCORELimit = _wtoi(str);
	if (m_nCORELimit < 0)
	{
		m_nCORELimit = abs(m_nCORELimit);
	}

	// CO2相对误差限值
	str = si.GetString(L"LimitOfAnaGasCheck", L"CO2RELimit", L"5");
	ASSERT(L"" != str);
	m_nCO2RELimit = _wtoi(str);
	if (m_nCO2RELimit < 0)
	{
		m_nCO2RELimit = abs(m_nCO2RELimit);
	}

	// NO相对误差限值
	str = si.GetString(L"LimitOfAnaGasCheck", L"NORELimit", L"8");
	ASSERT(L"" != str);
	m_nNORELimit = _wtoi(str);
	if (m_nNORELimit < 0)
	{
		m_nNORELimit = abs(m_nNORELimit);
	}
}

void CNHA503XGasCheckDlg2::ImportGasCheckParams(void)
{
	CString strPath;
	// 获取exe(dll)文件绝对路径
	GetModuleFileName(NULL, strPath.GetBuffer(MAX_PATH), MAX_PATH);
...
void CNHA503XGasCheckDlg2::ExportGasCheckRTData(void)
{
...
void CNHA503XGasCheckDlg2::ExportGasCheckResult(void)
{
	CString strPathOfDataExchange;
	// 获取exe(dll)文件绝对路径
	GetModuleFileName(NULL, strPathOfDataExchange.GetBuffer(MAX_PATH), MAX_PATH);
...
void CNHA503XGasCheckDlg2::OpenSerialPortAndTestCommunication(void)
{
	CString strPath;
	// 获取exe(dll)文件绝对路径
	GetModuleFileName(NULL, strPath.GetBuffer(MAX_PATH), MAX_PATH);
...
void CNHA503XGasCheckDlg2::ResetProcess()
{
...
void CNHA503XGasCheckDlg2::ClearRTDisplay(void)
{
	// HC实时值
	m_lbHC.SetText(L"--");
	// C3H8实时值
...
void CNHA503XGasCheckDlg2::OnBnClickedButtonQuit()
{
	// TODO: 在此添加控件通知处理程序代码
...
void CNHA503XGasCheckDlg2::OnBnClickedButtonStart()
{
	// TODO: 在此添加控件通知处理程序代码
...
void CNHA503XGasCheckDlg2::GasCheckProcess(void)
{
	if (0 == m_nStep)
	{
		// 重置流程
...
DWORD CNHA503XGasCheckDlg2::GetNominalValue(void)
{
...
void CNHA503XGasCheckDlg2::SaveNominalValueToIniFile(void)
{
	///////////////////////////////////////////////////////////// 记录到配置文件start
	CString strPath;
	// 获取exe(dll)文件绝对路径
...
void CNHA503XGasCheckDlg2::CalculateResult(void)
{
	// 获取一次最新数据进行结果判断
	// 主数据
	short sHC(0);
...
void CNHA503XGasCheckDlg2::ShowResult(void)
{
	CString str;
	CString strFormat;
...
void CNHA503XGasCheckDlg2::OnBnClickedButtonStop()
{
	// TODO: 在此添加控件通知处理程序代码
...
void CNHA503XGasCheckDlg2::OnTimer(UINT_PTR nIDEvent)
{
...
void CNHA503XGasCheckDlg2::SetProcessCtrlCallBack(DWORD (CALLBACK * pfProcessCtrl)(const DWORD dwStatus, const void* pBuf, const LONG lBufLen))
{
	m_pfProcessCtrl = pfProcessCtrl;
}

void CNHA503XGasCheckDlg2::OnStnClickedStaticNoRe()
{
	// TODO: 在此添加控件通知处理程序代码
}
