...
CPlotData::CPlotData(CString szName, COLORREF crColor, int nStyle, std::vector<POINTF> *pvecData)
{
	m_szName = szName;
	m_nStyle = nStyle;
	m_oIsDirty = true;
	...
	m_pptScreen = NULL;
	m_uiPointCount = 0;
	m_uiMaxScreenPoint = 0;

	//  copy the point data over, calculating max & min.
	UINT uiPointCount = UINT(pvecData->size());
	if (uiPointCount>0)
	{
		m_pptScreen = new POINT[uiPointCount];
		m_uiPointCount = uiPointCount;
		if (m_pptData.empty() && m_pptScreen!=NULL)
		{
			UpdateDatas(pvecData);
		}
	}
	//CPlotData(szName,crColor,nStyle,pvecData,uiPointCount);
}
/////////////////////////////////////////////////////////////////////////////

CPlotData::CPlotData(CString szName, COLORREF crColor, int nStyle, POINTF *pptData, UINT uiPointCount)
{
	m_szName = szName;
	m_nStyle = nStyle;
	m_oIsDirty = true;
	...
	if (uiPointCount>0)
	{
		...
		if (m_pptData.empty() && m_pptScreen!=NULL)
		{
			UpdateDatas(pptData,uiPointCount);
		}
	}
}
/////////////////////////////////////////////////////////////////////////////
...
CPlotData& CPlotData::operator=(const CPlotData& pd)
{
	...
	if (pd.m_uiPointCount>0)
	{
		m_pptData.resize(pd.m_uiPointCount);
		m_pptScreen = new POINT[pd.m_uiPointCount];
		m_uiPointCount = pd.m_uiPointCount;
		if (m_pptData.empty() && m_pptScreen!=NULL)
		{
...
bool CPlotData::UpdateDatas(POINTF *pptData, UINT uiPointCount)
{
	m_uiPointCount = uiPointCount;
	m_pptData.resize(m_uiPointCount);

	//SetPtScreenSize(m_uiPointCount);
...
bool CPlotData::UpdateDatas(const std::vector<POINTF>* pvecData)
{
	m_uiPointCount = UINT(pvecData->size());
	m_pptData.resize(m_uiPointCount);
...
bool CPlotData::AddData(POINTF ptData)
{
	m_pptData.push_back(ptData);

	double xAccum = m_eYDataMean*m_uiPointCount;
	xAccum += ptData.y;
	m_eXDataMin = (ptData.x<m_eXDataMin) ? (ptData.x) : (m_eXDataMin);
	m_eXDataMax = (ptData.x>m_eXDataMax) ? (ptData.x) : (m_eXDataMax);
	m_eYDataMin = (ptData.y<m_eYDataMin) ? (ptData.y) : (m_eYDataMin);
	m_eYDataMax = (ptData.y>m_eYDataMax) ? (ptData.y) : (m_eYDataMax);
	
	m_uiPointCount+=1;
	//CheckPtScreenSize(m_uiPointCount);
...
void CPlotData::SetPtScreenSize(UINT uiCount)
{
	m_uiMaxScreenPoint = uiCount;
	if(m_pptScreen!=NULL)
		delete [] m_pptScreen;
	if(m_uiMaxScreenPoint>0)
		m_pptScreen = new POINT[m_uiMaxScreenPoint];
}
/////////////////////////////////////////////////////////////////////////////
...
BOOL CLinePlot::Create(const RECT &rect,CWnd *pParentWnd, UINT uiID)
{
	BOOL oOkay = CWnd::Create(NULL, _T("LinePlot"), WS_CHILD | WS_VISIBLE, rect, pParentWnd, uiID);
...
void CLinePlot::DrawPlotData(CDC* pDC, UINT uiIndex, CRect &rcBounds)
{
	...
	{
		if(m_lstPlotData[uiIndex].m_uiMaxScreenPoint<=m_lstPlotData[uiIndex].m_uiPointCount)// 改变屏幕显示曲线大小
		{
			m_lstPlotData[uiIndex].SetPtScreenSize( UINT(m_lstPlotData[uiIndex].m_uiPointCount*1.2) );
		}
	...
	case LpLine:
		{
			...
			CPen penPlot(PS_SOLID, 2, m_lstPlotData[uiIndex].m_crColor);
...
CRect CLinePlot::DrawFramework(CDC *pDC, CRect rcBounds, int nFontHeight, bool oIsPrinting)
{
	...
	lf.lfWeight = FW_BOLD;
	lf.lfHeight =LONG(lf.lfHeight * 1.25f);
	...
	//  创建标尺画笔(灰)
	CPen penGray(PS_SOLID, 1, m_crGridColor);
	//  创建外框画笔(黑)
	CPen penBlack(PS_SOLID, 1, m_crFrameworkColor);


	//  获取标题字体高度.
	int nCaptionHeight = nFontHeight + 2;
	...
	CString szScaleTest(_T('9'), nNumChars+3);
	...
	int nCount = (int)m_lstPlotData.size();
	if (nCount>1)
	{
		...
		m_rcKeyArea.top = nYCenter - ((siScale.cy+4) * nCount) / 2;
		m_rcKeyArea.bottom = nYCenter + ((siScale.cy+4) * nCount) / 2;
	...
	CString szXFormat;
	szXFormat.Format(_T("%%0.%uf"), m_uiXPrecision-1);
	CString szYFormat;
	...
	*/
	...
	if (rcPlotArea.left < rcPlotArea.right)
	{
		...
		POINT ptXScale;
		POINT ptYScale;
		ptXScale.x = ptXScale.y = 0;
		ptYScale.x = ptYScale.y = 0;
		CString szScale;
		pfonOld = pDC->SelectObject(&fonScale);
...
/*
		...
		pDC->FillSolidRect(rcXLockBottom, m_crFrameworkColor);
		...
		pDC->FillSolidRect(rcYLockBottom, m_crFrameworkColor);
		pDC->SetBkColor(m_crBkColor);
		//  draw the lock top.
...
*/
		...
		if (nCount>1)
		{
			...
			for (int ii=0; ii<nCount; ii++)
			{
				...
				rcKey.right = m_rcKeyArea.right+10;
		...
		if (nCount>0 && m_nSelected>=0 && m_nSelected<nCount)
		{
			if (m_nCursor >= 0 && (UINT)(m_nCursor) < m_lstPlotData[m_nSelected].m_uiPointCount)
			{
				...
				szPosition.Format(_T("Position: %0.3f    Value: %0.3f"),
			...
			szPosition.Format(_T("Mean: %0.3f    Min: %0.3f    Max: %0.3f"),
				eYMean, eYMin, eYMax);
			pDC->SetTextAlign(TA_RIGHT | TA_BOTTOM);
...
void CLinePlot::DrawData(CDC *pDC, CRect rcBounds, bool oIsPrinting) 
{
	...
	CPen penDotBlack(PS_DOT, 1, m_crFrameworkColor);
...
void CLinePlot::OnPaint() 
{
	...
	//字体颜色
	dcMem.SetTextColor(m_crFrameworkColor);
	dcMem.SetBkColor(m_crBkColor);
	//dcMem.SetBkMode(TRANSPARENT);
	//背景颜色
	dcMem.FillSolidRect(&rcClient,m_crBkColor);
	...
	if(dlgPrint.DoModal() == IDOK)
	{
		...
		if (hdcPrinter!=NULL)
		{
			...
			if (dcPrinter.StartDoc(&diPrinter) >= 0)
			{
				...
				int nFontHeight = int(nPrinterHeight / 80.0f);
...
int CLinePlot::AddLine(CString szName, COLORREF crColor, enumPlotStyle nStyle, POINTF *pptData, UINT uiPointCount)
{
	//  create a new plot
	CPlotData PlotData(szName, crColor, (int)(nStyle), pptData, uiPointCount);
	return AddLine(PlotData);
}
/////////////////////////////////////////////////////////////////////////////

int CLinePlot::AddLine(CString szName, COLORREF crColor, enumPlotStyle nStyle, std::vector<POINTF> *pvecData)
{
	//  create a new plot
...
int CLinePlot::AddLine(const CPlotData &PlotData)
{
	m_lstPlotData.push_back(PlotData);

	m_nSelected = int(m_lstPlotData.size()-1);//选择新添加的曲线
	//  should the x-axis limits be adjusted?
	UINT nCount = UINT(m_lstPlotData.size());
...
bool CLinePlot::Remove(int nIndex)
{
	//  remove a plot.
	bool oRetVal = false;
	if (nIndex<0 || nIndex>=(int)m_lstPlotData.size())
		return false;
	else
	{
		...
		{
			m_nSelected--;
			int nMaxIndex = int(m_lstPlotData.size() - 1);
...
bool CLinePlot::AddData(int nIndex, POINTF ptData)
{
	m_lstPlotData[nIndex].AddData(ptData);

	bool bDirty=0;
	if (m_oIsXLocked == false)
	{
		if (m_lstPlotData[nIndex].m_eXDataMin < m_eXMin)
		{
			m_eXMin = m_lstPlotData[nIndex].m_eXDataMin;
			bDirty = 1;
		}
		if (m_lstPlotData[nIndex].m_eXDataMax > m_eXMax)
		{
			m_eXMax = float(m_lstPlotData[nIndex].m_eXDataMax*1.2);
			bDirty = 1;
		}
	...
	if (m_oIsYLocked == false)
	{
		if (m_lstPlotData[nIndex].m_eYDataMin < m_eYMin)
		{
			m_eYMin = m_lstPlotData[nIndex].m_eYDataMin;
			bDirty = 1;
		}
		if (m_lstPlotData[nIndex].m_eYDataMax > m_eYMax)
		{
			m_eYMax = float(m_lstPlotData[nIndex].m_eYDataMax*1.2);
			bDirty = 1;
		}
	}
...
*/ // 缩放end
	...
	{
		szFormat.Format(_T("%%0.%uf"), m_uiXPrecision);
	...
	{
		szFormat.Format(_T("%%0.%uf"), m_uiYPrecision);
...
void CLinePlot::HideEditWindow()
{
	...
	if (m_edtDataEntry.IsWindowVisible()==TRUE)
	{
		...
		float eValue  = (float)(_wtof(szText.GetBuffer(1)));
...
std::vector<POINTF> *CLinePlot::GetDatas(int nIndex)
{
	return &m_lstPlotData[nIndex].m_pptData;
}
/////////////////////////////////////////////////////////////////////////////

bool CLinePlot::SetDatas(int nIndex, POINTF *pptData, UINT uiPointCount)
{
	m_lstPlotData[nIndex].UpdateDatas(pptData,uiPointCount);
	if (m_hWnd!=NULL)
		Invalidate();
	return true;
}
...
bool CLinePlot::SetDatas(int nIndex, std::vector<POINTF>* pvecData)
{
	m_lstPlotData[nIndex].UpdateDatas(pvecData);
	if (m_hWnd!=NULL)
		Invalidate();
	return true;
}
...
void CLinePlot::SetBkColor(COLORREF crColor)
{
	m_crBkColor = crColor;
}
/////////////////////////////////////////////////////////////////////////////

void CLinePlot::SetFrameworkColor(COLORREF crColor)
{
	m_crFrameworkColor = crColor;
}
/////////////////////////////////////////////////////////////////////////////

void CLinePlot::SetGridColor(COLORREF crColor)
{
