...
* 文件名称：NanhuaNht1L.h
* 文件标识：
* 摘要：不透光度计NHT1L通讯类，继承于抽象类COpa
*		依照不透光度计NHT1L通讯协议进行编写
*       具体请参考转不透光度计NHT1L通讯协议说明书
*		
*
* 当前版本：1.0
* 作者：Cui,hyh
...
* 完成日期：2010-08-26
* 作者：Raylee
* 描述：添加GetTestResultData()函数，此种设备不支持，返回0x15
*
* 当前版本：1.0.1
* 完成日期：2010-04-13
* 作者：Cui
* 描述：修正GetRealTimeData函数bug：取K值出错
*
* 当前版本：1.0.0
* 完成日期：2009-12-08
* 作者：Cui,hyh
...
#ifndef NANHUA_NHT1L_H
#define NANHUA_NHT1L_H

#include "Opa.h"

class CNanhuaNht1L : public COpa
{
public:
	CNanhuaNht1L(void);
	~CNanhuaNht1L(void);
...
public:
	...
	DWORD EntrySensitivityCalStatus(void);
	...
	// OPA_WR_FUNC_EQUIP_RETURE_0X15
	// OPA_WR_FUNC_CHECK_SUM_FAIL
	DWORD ProceedSensitivityCal(float const pfValue);
	...
	// OPA_WR_FUNC_EQUIP_RETURE_0X15
	// OPA_WR_FUNC_CHECK_SUM_FAIL
	DWORD QuitSensitivityCalStatus(void);
