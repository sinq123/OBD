...
#ifndef FULI_FLA502_H
#define FULI_FLA502_H

#include "Ana.h"

class CFuLiFLA502 : public CAna
{
public:
	CFuLiFLA502(void);
	~CFuLiFLA502(void);
...
public:
	...
	// (out)pbWaterLevel,判断水位是否已满,单位:无,可以为空指针（新）
	// 返回函数执行结果：
	// ANA_WR_SUCCEED
	...
	// ANA_WR_EQUIP_RETURE_0X15
	// ANA_WR_CHECK_SUM_FAIL
	DWORD OpenCalibrationGas(BYTE *const pbResult = NULL);
	...
	// ANA_WR_EQUIP_RETURE_0X15
	// ANA_WR_CHECK_SUM_FAIL
	DWORD CloseCalibrationGas(BYTE *const pbResult = NULL);
	...
	// ANA_WR_EQUIP_RETURE_0X15
	// ANA_WR_CHECK_SUM_FAIL
	DWORD OpenZeroAirGas(BYTE *const pbResult = NULL);
	...
	// ANA_WR_EQUIP_RETURE_0X15
	// ANA_WR_CHECK_SUM_FAIL
	DWORD CloseZeroAirGas(BYTE *const pbResult = NULL);
	...
	// ANA_WR_EQUIP_RETURE_0X15
	// ANA_WR_CHECK_SUM_FAIL
	DWORD SetNumberOfcylinders(BYTE *const pbResult, const int Number);
	...
	// ANA_WR_EQUIP_RETURE_0X15
	// ANA_WR_CHECK_SUM_FAIL
	DWORD SetPositionToUSB(BYTE *const pbResult = NULL);
	...
	// ANA_WR_EQUIP_RETURE_0X15
	// ANA_WR_CHECK_SUM_FAIL
	DWORD SetPositionTobranch(BYTE *const pbResult = NULL);
	// 43设置燃料类型为汽油
	// (out)pbResult,仪器执行结果指针,一般返回0x06,可以为空指针
	// 返回函数执行结果：
	// ANA_WR_SUCCEED
	...
	//流量计温度校准：不支持
	DWORD SetCalibrationTemp_Flowmeter(BYTE *const pbResult, const float fCalTemp){return ANA_WR_EQUIP_RETURE_0X15;};
	DWORD TempCalibration_Flowmeter(BYTE *const pbResult, BYTE *const pbCalResult){return ANA_WR_EQUIP_RETURE_0X15;};
