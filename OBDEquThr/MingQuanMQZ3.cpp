...
CMingQuanMQZ3::CMingQuanMQZ3(void)
...
// 打开串口
DWORD CMingQuanMQZ3::Open(BYTE bPort)
{
	// 关闭已存在的串口句柄
	Close();
...
bool CMingQuanMQZ3::IsOpen(void) const
{
...
// 关闭串口
DWORD CMingQuanMQZ3::Close(void)
{
	if (IsOpen())
	{
		// 进入串口操作临界区
...
DWORD CMingQuanMQZ3::GetRev(int *pnRev)
{
	assert(NULL != pnRev);
