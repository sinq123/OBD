...
* 完成日期：2010-07-05
* 作者：Hyh
* 描述：1)为兼容2.4版本分析仪协议,历史机器取命令返回慢,现将收数据超时时间设为2s,原为1s
*       2)取主数据命令由0x01更变为0x81
*
* 当前版本：1.0.3
* 完成日期：2010-06-21
* 作者：Hyh
...
#ifndef NANHUA_NHA503_H
#define NANHUA_NHA503_H

#include "Ana.h"

class CNanhuaNha503 : public CAna
{
public:
	CNanhuaNha503(void);
	~CNanhuaNha503(void);
...
public:
	...
	// (out)pbWaterFull,判断水位是否已满,单位:无,可以为空指针（新）
	// 返回函数执行结果：
	// ANA_WR_SUCCEED
	...
	//// 获取当前使用的操作模式
	//DWORD GetOperationModeUsedNow(void) const;
...
private:
	...
	// 返回，函数执行结果：
	// NHA503X_WR_SUCCEED
	// NHA503X_WR_PORT_NOT_OPEN
	// NHA503X_WR_READ_SPECIFIED_DATA_FAIL
	// NHA503X_WR_EQUIP_RETURE_0X15
	// NHA503X_WR_CHECK_SUM_FAIL
