...
// 打开串口
DWORD CFuLiFLA502::Open(BYTE bPort)
{
	// 关闭已存在的串口句柄
	Close();
...
bool CFuLiFLA502::IsOpen(void) const
{
	return m_hComm != INVALID_HANDLE_VALUE;
}

DWORD CFuLiFLA502::WriteAndReadData(BYTE *const pbWriteBuffer, 
...
BYTE CFuLiFLA502::CheckSum(const BYTE *const pb, const UINT un)
{
...
DWORD CFuLiFLA502::WriteAndReadData_Flowmeter(BYTE *const pbWriteBuffer, 
												 const DWORD dwWriteLength, 
												 BYTE *const pbReadBuffer, 
												 const DWORD dwReadLength, 
												 const bool bNeedWriteChkSum, 
...
WORD CFuLiFLA502::CheckSum_Flowmeter(const BYTE *const pb, const UINT un)
{
	assert(NULL != pb);
	assert(0 != un);
...
DWORD CFuLiFLA502::CmdFormat_OnlyReturn06(const BYTE bCmd, BYTE *const pbResult)
{
	BYTE bWriteBuf = bCmd;
	BYTE bReadBuf(0);
...
DWORD CFuLiFLA502::CmdFormat_Return00_01_05(const BYTE bCmd, BYTE *const pbResult)
{
	BYTE bWriteBuf = bCmd;
	BYTE bReadBuf(0);
	DWORD dw = WriteAndReadData(&bWriteBuf, 1, &bReadBuf, 1, false, false, true);
...
DWORD CFuLiFLA502::CmdFormat_Return00_01_05(const BYTE bCmd, BYTE *const pbResult, short *const psHC, float *const pfCO, float *const pfCO2, float *const pfO2, short *const psNO, float *const pfPef)
{
...
DWORD CFuLiFLA502::CmdFormat_Return06_00_01(const BYTE bCmd, BYTE *const pbResult, const USHORT usC3H8, const float fCO, const float fCO2, const USHORT usNO)
{
	assert(NULL != pbResult);
...
DWORD CFuLiFLA502::CmdFormat_Return06_00_01(const BYTE bCmd, BYTE *const pbResult, const float fValue)
{
	assert(NULL != pbResult);
...
DWORD CFuLiFLA502::CmdFormat_Return00_01(const BYTE bCmd, BYTE *const pbResult, const USHORT usC3H8, const float fCO, const float fCO2, const USHORT usNO)
{
	assert(NULL != pbResult);
	assert(usC3H8 >= 0);
	assert(fCO >= 0.0f);
	...
	bWriteBuf[3] = HIBYTE((WORD)(fCO * 1000.0f + 0.5f));
	bWriteBuf[4] = LOBYTE((WORD)(fCO * 1000.0f + 0.5f));
...
DWORD CFuLiFLA502::CmdFormat_Return00_01_02(const BYTE bCmd, BYTE *const pbResult, const float fValue)
{
	assert(NULL != pbResult);
...
// 1取主数据
DWORD CFuLiFLA502::GetMajorData(short *const psHC, 
							float *const pfCO, 
							float *const pfCO2, 
							float *const pfO2, 
							short *const psNO,
							...
							bool *const pbWaterLevel)
{
	...
	if (ANA_WR_SUCCEED == dw)
	{
		...
		{
			// 可以为正或者负数,添加short强制转换
			*pfCO = (short)(MAKEWORD(bReadBuf[4], bReadBuf[3])) / 1000.0f;
		}
		// (3) CO2
		if (NULL != pfCO2)
		{
...
DWORD CFuLiFLA502::GetMinorData(float *const pfET, float *const pfAP, float *const pfRH)
{
	assert(NULL != pfET);
	assert(NULL != pfAP);
	assert(NULL != pfRH);
...
// 3抽空气
DWORD CFuLiFLA502::DeflateEnvAir(BYTE *const pbResult)
{
	return CmdFormat_OnlyReturn06(3, pbResult);
}

// 4抽管道气(开泵,气样从探头进入)
DWORD CFuLiFLA502::DeflatePipeAir(BYTE *const pbResult)
{
	return CmdFormat_OnlyReturn06(4, pbResult);
}

// 5清洗管道(反吹)
DWORD CFuLiFLA502::InverseBlow(BYTE *const pbResult)
{
	return CmdFormat_OnlyReturn06(5, pbResult);
}

// 6停止清洗(停止反吹及停泵)
DWORD CFuLiFLA502::StopPump(BYTE *const pbResult)
{
	return CmdFormat_OnlyReturn06(6, pbResult);
}

// 7检漏
DWORD CFuLiFLA502::CheckLeak(BYTE *const pbResult)
{
	return CmdFormat_Return00_01_05(7, pbResult);
}

// 8调零
DWORD CFuLiFLA502::Zero(BYTE *const pbResult)
{
	return CmdFormat_Return00_01_05(8, pbResult);
}

// 9写入校准气浓度
DWORD CFuLiFLA502::SetCalibrationGasConcentration(BYTE *const pbResult, const int nPoint, 
...
// 10校准
DWORD CFuLiFLA502::GasCalibration(BYTE *const pbResult, const int nChanel, 
							  short *const psHC, 
							  float *const pfCO, 
							  float *const pfCO2, 
							  float *const pfO2, 
...
// 11写入检查气浓度
DWORD CFuLiFLA502::SetCheckGasConcentration(BYTE *const pbResult, const USHORT usChkC3H8, const float fChkCO, const float fChkCO2, const USHORT usChkNO)
{
	return CmdFormat_Return00_01(11, pbResult, usChkC3H8, fChkCO, fChkCO2, usChkNO);
}

// 12自动检查
DWORD CFuLiFLA502::GasCheck(BYTE *const pbResult, 
						short *const psHC, 
						float *const pfCO, 
						float *const pfCO2, 
						float *const pfO2, 
...
// 13取PEF
DWORD CFuLiFLA502::GetPef(float *const pfPef)
{
	assert(NULL != pfPef);
...
// 14清除NO老化标志
// 503特有
DWORD CFuLiFLA502::ClearNoSign(BYTE *const pbResult)
{
	return CmdFormat_OnlyReturn06(14, pbResult);
}

// 15设置调零气体为空气
DWORD CFuLiFLA502::SetZeroAirTypeToEnvAir(BYTE *const pbResult)
{
	return CmdFormat_OnlyReturn06(15, pbResult);
}

// 16设置调零气体为零气
DWORD CFuLiFLA502::SetZeroAirTypeToZeroAir(BYTE *const pbResult)
{
	return CmdFormat_OnlyReturn06(16, pbResult);
}

// 17设置冲程为4
DWORD CFuLiFLA502::SetStrokeTypeToFourStroke(BYTE *const pbResult)
{
	return CmdFormat_OnlyReturn06(17, pbResult);
}

// 18设置冲程为2
DWORD CFuLiFLA502::SetStrokeTypeToTwoStroke(BYTE *const pbResult)
{
	return CmdFormat_OnlyReturn06(18, pbResult);
}

// 19设置点火方式为单次
DWORD CFuLiFLA502::SetFireTypeToSingleFire(BYTE *const pbResult)
{
	return CmdFormat_OnlyReturn06(19, pbResult);
}

// 20设置点火方式为二次
DWORD CFuLiFLA502::SetFireTypeToDoubleFire(BYTE *const pbResult)
{
	return CmdFormat_OnlyReturn06(20, pbResult);
}

// 21允许自动调零
DWORD CFuLiFLA502::EnableAutoZero(BYTE *const pbResult)
{
	return CmdFormat_OnlyReturn06(21, pbResult);
}

// 22禁止自动调零
DWORD CFuLiFLA502::DisableAutoZero(BYTE *const pbResult)
{
...
// 27环境温度校准
DWORD CFuLiFLA502::ETCalibration(BYTE *const pbResult, const float fCalET)
{
	//assert(NULL != pbResult);
...
// 29油温校准
DWORD CFuLiFLA502::OTCalibration(BYTE *const pbResult, const float fCalOT)
{
	//assert(NULL != pbResult);
...
// 30环境湿度校准
DWORD CFuLiFLA502::RHCalibration(BYTE *const pbResult, const float fCalRH)
{
	//assert(NULL != pbResult);
...
// 31大气压力校准
DWORD CFuLiFLA502::APCalibration(BYTE *const pbResult, const float fCalAP)
{
	//assert(NULL != pbResult);
...
// 32检查气开
DWORD CFuLiFLA502::OpenCheckGas(BYTE *const pbResult)
{
	return CmdFormat_OnlyReturn06(32, pbResult);
}

// 33检查气关
DWORD CFuLiFLA502::CloseCheckGas(BYTE *const pbResult)
{
	return CmdFormat_OnlyReturn06(33, pbResult);
}

// 34取仪器状态
DWORD CFuLiFLA502::GetStatus(BYTE *const pbStatus)
{
...
// 35取预热剩余时间
DWORD CFuLiFLA502::GetWarmUpTimeLeft(USHORT *const pusSecondsLeft)
{
...
// 36标定气开
DWORD CFuLiFLA502::OpenCalibrationGas(BYTE *const pbResult)
{
	return CmdFormat_OnlyReturn06(36, pbResult);
}

// 37标定气关
DWORD CFuLiFLA502::CloseCalibrationGas(BYTE *const pbResult)
{
	return CmdFormat_OnlyReturn06(37, pbResult);
}
// 38零气开
DWORD CFuLiFLA502::OpenZeroAirGas(BYTE *const pbResult)
{
	return CmdFormat_OnlyReturn06(38, pbResult);
}

// 39零气关
DWORD CFuLiFLA502::CloseZeroAirGas(BYTE *const pbResult)
{
	return CmdFormat_OnlyReturn06(39, pbResult);
}

// 40设置汽缸数
DWORD CFuLiFLA502::SetNumberOfcylinders(BYTE *const pbResult, const int Number)
{
	assert(NULL != pbResult);
	BYTE bReadBuf(0);
	BYTE bWriteBuf[2];
	...
	bWriteBuf[0] = 40;
	bWriteBuf[1] = LOBYTE((WORD)Number);
	
	DWORD dw = WriteAndReadData(bWriteBuf, 2, &bReadBuf, 1, true, false, true);
	if (Number == dw)
	{
		// 仪器返回结果
		*pbResult = bReadBuf;
...
// 41设置测量 位置为总线
DWORD CFuLiFLA502::SetPositionToUSB(BYTE *const pbResult)
{
	return CmdFormat_OnlyReturn06(41, pbResult);
}
// 42设置测量 位置为分线
DWORD CFuLiFLA502::SetPositionTobranch(BYTE *const pbResult)
{
	return CmdFormat_OnlyReturn06(42, pbResult);
}
// 43设置燃料类型为汽油
DWORD CFuLiFLA502::SetFuelTypeToGas(BYTE *const pbResult)
{
...
// 44设置燃料类型为液化石油气
DWORD CFuLiFLA502::SetFuelTypeToLPG(BYTE *const pbResult)
{
	return CmdFormat_OnlyReturn06(44, pbResult);
}

// 45设置燃料类型为天然气
DWORD CFuLiFLA502::SetFuelTypeToNG(BYTE *const pbResult)
{
	return CmdFormat_OnlyReturn06(45, pbResult);
}

// 46设置燃料类型为乙醇汽油
DWORD CFuLiFLA502::SetFuelTypeToGasohol(BYTE *const pbResult)
{
	return CmdFormat_OnlyReturn06(46, pbResult);
}

DWORD CFuLiFLA502::SetOperationMode(const enum OperationMode op, BYTE *const pbResult/*=NULL*/)
{
//#ifdef _DEBUG
//	SYSTEMTIME st;
//	GetSystemTime(&st);
...
DWORD CFuLiFLA502::GetProcessStatus_Flowmeter(BYTE *const pbResult, WORD *const pwProcessStatus, WORD *const pwProcess)
{
...
DWORD CFuLiFLA502::StartCollectData_Flowmeter(const WORD wCollectionTime/*=195*/, WORD *const pwMiniStatus/*=NULL*/, BYTE *const pbResult/*=NULL*/)
{
//#ifdef _DEBUG
//	SYSTEMTIME st;
//	GetSystemTime(&st);
...
DWORD CFuLiFLA502::StopCollectData_Flowmeter(WORD *const pwMiniStatus/*=NULL*/, BYTE *const pbResult/*=NULL*/)
{
//#ifdef _DEBUG
//	SYSTEMTIME st;
//	GetSystemTime(&st);
...
DWORD CFuLiFLA502::GetProcessDataStatus_WT_Flowmeter(const WORD wCntStart,
...
DWORD CFuLiFLA502::GetProcessDataStatus_CT_Flowmeter(const WORD wCntStart,
		const WORD wCntLength,
		short *const psHCct, 
		float *const pfCOct, 
		float *const pfCO2ct, 
...
DWORD CFuLiFLA502::GetTotalWeight_Flowmeter(USHORT *const pusHCwt, 
...
DWORD CFuLiFLA502::GetCollectedDataNumber_Flowmeter(WORD *const pwCorrectedDataNumber, BYTE *const pbResult/*=NULL*/)
{
...
DWORD CFuLiFLA502::GetRealTimeData_Flowmeter(float *const pfFluxStd0, float *const pfFluxStd20, float *const pfFluxAct, float *const pfPres, float *const pfUDilO2, float *const pfGasTemp/*=NULL*/, BYTE *const pbResult/*=NULL*/)
{
	//assert(NULL!=pfFlux);
	//assert(NULL!=pfPres);
...
DWORD CFuLiFLA502::GetRealTimeData_Flowmeter_ALL(WORD * const pwStatus0,WORD * const pwStatus1,WORD * const pwStatus2,WORD * const pwStatus3,WORD * const pwStatus4,WORD * const pwStatus5,WORD * const pwStatus6,
...
DWORD CFuLiFLA502::SetCalibrationFlux_Flowmeter(BYTE *const pbResult, const enum FluxCalibrationMode cm, const float fCalFlux)
{
	assert(NULL != pbResult);
	...
	DWORD dw;
	if (SinglePointFlowCal == cm)
	{
		dw = WriteAndReadData_Flowmeter(bWriteBuf, 12, bReadBuf, 6, true, true);
	}
	else
	{
		dw = WriteAndReadData_Flowmeter(bWriteBuf, 18, bReadBuf, 6, true, true);
	}
...
DWORD CFuLiFLA502::FluxCalibration_Flowmeter(BYTE *const pbResult, BYTE *const pbCalResult, const enum FluxCalibrationMode cm)
{
	assert(NULL != pbResult);
...
DWORD CFuLiFLA502::SetCalibrationPres_Flowmeter(BYTE *const pbResult, const float fCalPres)
{
	assert(NULL != pbResult);
...
DWORD CFuLiFLA502::PresCalibration_Flowmeter(BYTE *const pbResult, BYTE *const pbCalResult)
{
	assert(NULL != pbResult);
	assert(NULL != pbCalResult);
...
DWORD CFuLiFLA502::SetCalibrationDilO2_Flowmeter(BYTE *const pbResult, const float fCalDilO2)
{
	assert(NULL != pbResult);
...
DWORD CFuLiFLA502::DilO2Calibration_Flowmeter(BYTE *const pbResult, BYTE *const pbCalResult, bool bDataRequestMode)
{
	assert(NULL != pbResult);
	assert(NULL != pbCalResult);
...
DWORD CFuLiFLA502::SetCalibrationDilO2_Flowmeter_High(BYTE *const pbResult, const float fCalDilO2)
{
	assert(NULL != pbResult);
	assert(fCalDilO2>10e-6f && fCalDilO2<100.0f);
...
DWORD CFuLiFLA502::DilO2Calibration_Flowmeter_High(BYTE *const pbResult, BYTE *const pbCalResult, bool bDataRequestMode)
{
	assert(NULL != pbResult);
	assert(NULL != pbCalResult);
...
DWORD CFuLiFLA502::SetCalibrationDilO2_Flowmeter_Middle(BYTE *const pbResult, const float fCalDilO2)
{
	assert(NULL != pbResult);
	assert(fCalDilO2>10e-6f && fCalDilO2<100.0f);
...
DWORD CFuLiFLA502::DilO2Calibration_Flowmeter_Middle(BYTE *const pbResult, BYTE *const pbCalResult, bool bDataRequestMode)
{
	assert(NULL != pbResult);
	assert(NULL != pbCalResult);
...
DWORD CFuLiFLA502::SetCalibrationDilO2_Flowmeter_Low(BYTE *const pbResult, const float fCalDilO2)
{
	assert(NULL != pbResult);
	assert(fCalDilO2>10e-6f && fCalDilO2<100.0f);
...
DWORD CFuLiFLA502::DilO2Calibration_Flowmeter_Low(BYTE *const pbResult, BYTE *const pbCalResult, bool bDataRequestMode)
{
	assert(NULL != pbResult);
	assert(NULL != pbCalResult);
...
DWORD CFuLiFLA502::GetHCCOCO2DelayTime_Flowmeter(BYTE *const pbResult, float *const pfDelayTime)
{
	assert(NULL != pbResult);
...
DWORD CFuLiFLA502::SetHCCOCO2DelayTime_Flowmeter(BYTE *const pbResult, const float fDelayTime)
{
	assert(NULL != pbResult);
...
DWORD CFuLiFLA502::HCCOCO2DelayTime_Flowmeter(BYTE *const pbResult, BYTE *const pbCalResult)
{
	assert(NULL != pbResult);
	assert(NULL != pbCalResult);
...
DWORD CFuLiFLA502::GetO2DelayTime_Flowmeter(BYTE *const pbResult, float *const pfDelayTime)
{
	assert(NULL != pbResult);
	assert(NULL != pfDelayTime);
...
DWORD CFuLiFLA502::SetO2DelayTime_Flowmeter(BYTE *const pbResult, const float fDelayTime)
{
	assert(NULL != pbResult);
	assert(fDelayTime>10e-6f && fDelayTime<100.0f);
...
DWORD CFuLiFLA502::O2DelayTime_Flowmeter(BYTE *const pbResult, BYTE *const pbCalResult)
{
	assert(NULL != pbResult);
	assert(NULL != pbCalResult);
...
DWORD CFuLiFLA502::GetNOxDelayTime_Flowmeter(BYTE *const pbResult, float *const pfDelayTime)
{
	assert(NULL != pbResult);
	assert(NULL != pfDelayTime);
...
DWORD CFuLiFLA502::SetNOxDelayTime_Flowmeter(BYTE *const pbResult, const float fDelayTime)
{
	assert(NULL != pbResult);
	assert(fDelayTime>10e-6f && fDelayTime<100.0f);
...
DWORD CFuLiFLA502::NOxDelayTime_Flowmeter(BYTE *const pbResult, BYTE *const pbCalResult)
{
	assert(NULL != pbResult);
	assert(NULL != pbCalResult);
...
DWORD CFuLiFLA502::SatrtGetEnvironmentO2_Flowmeter(void)
{
...
DWORD CFuLiFLA502::GetEnvironmentO2_Flowmeter(BYTE *const pbResult, float *const pfEnvO2)
{
//#ifdef _DEBUG
//	SYSTEMTIME st;
//	GetSystemTime(&st);
...
DWORD CFuLiFLA502::ReadCalibrationData_Flowmeter(BYTE *const pbResult, const BYTE bCalMode, BYTE *const pbReadBuf, const DWORD dwBytesToRead)
{
	assert(NULL != pbResult);
...
DWORD CFuLiFLA502::RunFan_Flowmeter(const bool bRun, BYTE *const pbResult/*=NULL*/)
{
//#ifdef _DEBUG
//	SYSTEMTIME st;
//	GetSystemTime(&st);
...
// 获取手动检测的数据
DWORD CFuLiFLA502::GetTestResultData(short *const psHCHigh/*=NULL*/, 
