...
#ifndef MINGQUAN_MQW50A_H
#define MINGQUAN_MQW50A_H

#include "Ana.h"

class CMingQuanMQW50A : public CAna
{
public:
	CMingQuanMQW50A(void);
	~CMingQuanMQW50A(void);
...
public:
	...
	// 23允许键盘操作
	DWORD EnableKeyOperate(BYTE *const pbResult = NULL);

	// 24禁止键盘操作
	DWORD DisableKeyOperate(BYTE *const pbResult = NULL);
	...
	// (out)pfGasFluxStd:稀释气体标准流量,单位:L/s。不能为NULL，所指向的内存大小必须大于等于wCntLength
	// (out)pfExhaustFlux:废气流量,单位:L/s。不能为NULL，所指向的内存大小必须大于等于wCntLength
	...
	//流量计温度校准：不支持
	DWORD SetCalibrationTemp_Flowmeter(BYTE *const pbResult, const float fCalTemp){return ANA_WR_CHECK_SUM_FAIL;};
	DWORD TempCalibration_Flowmeter(BYTE *const pbResult, BYTE *const pbCalResult){return ANA_WR_CHECK_SUM_FAIL;};
...
protected:
	...
	DWORD CmdFormat_Return00_01_05(const BYTE bCmd, BYTE *const pbResult,const BYTE bType);
