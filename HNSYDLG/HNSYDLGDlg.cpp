...
CHNSYDLGDlg::CHNSYDLGDlg(CWnd* pParent /*=NULL*/)
	: CDialogEx(CHNSYDLGDlg::IDD, pParent)
	, m_strLogFilePath(L"")
{
	m_hIcon = AfxGetApp()->LoadIcon(IDR_MAINFRAME);
}

void CHNSYDLGDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, ID_ED_LINENUMBER2, m_edLineNumber);
...
BEGIN_MESSAGE_MAP(CHNSYDLGDlg, CDialogEx)
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_BUTTON_UPDATE_VEHICLE_LIST, &CHNSYDLGDlg::OnBnClickedButtonUpdateVehicleList)
	ON_BN_CLICKED(IDC_BTN_SETCONFIG, &CHNSYDLGDlg::OnBnClickedBtnSetconfig)
	ON_BN_CLICKED(IDC_BTN_UP, &CHNSYDLGDlg::OnBnClickedBtnUp)
END_MESSAGE_MAP()


// CHNSYDLGDlg 消息处理程序

BOOL CHNSYDLGDlg::OnInitDialog()
{
	...
	SetConfig();
	GetOBDJCY();
	if (m_edLincenseCode.GetWindowTextLengthW() <= 1)
	{
		GetAccessToken();
	}
...
//显示。
HCURSOR CHNSYDLGDlg::OnQueryDragIcon()
{
...
void CHNSYDLGDlg::OnBnClickedButtonUpdateVehicleList()
{
	// TODO: 在此添加控件通知处理程序代码
...
void CHNSYDLGDlg::OnBnClickedBtnSetconfig()
{
	// TODO: 在此添加控件通知处理程序代码
	SetConfig();
	GetOBDJCY();
...
void CHNSYDLGDlg::OnBnClickedBtnUp()
{
	// TODO: 在此添加控件通知处理程序代码
	SaveINIAndSQLAndUP();
}

void CHNSYDLGDlg::GetLogFilePath(void)
{
	// 日志文件所在文件夹路径
	...
	strFileName.Format(L"湖南邵阳OBD%s.log", odtNow.Format(L"%Y-%m-%d"));
	m_strLogFilePath = wchLogFileFolderPath;
	m_strLogFilePath += strFileName;
}

void CHNSYDLGDlg::SetConfig(void)
{
...
void CHNSYDLGDlg::GetOBDJCY(void)
{
...
bool CHNSYDLGDlg::GetAccessToken(void)
{
	CNHLogAPI::WriteLogEx(m_strLogFilePath, L"参数设置", L"获取联网令牌", L"");
	bool bRet(false);

	CString strURL, strLineNumber;
	m_edURL.GetWindowTextW(strURL);
	m_edLineNumber.GetWindowTextW(strLineNumber);

	if (!strURL.IsEmpty() && !strLineNumber.IsEmpty())
	{
		CString strTemp;
		char* pchURL(NULL);
		...
		ns1__getAccessTokenResponse sRes;

		std::wstring wstrLineNum;
		wstrLineNum = strLineNumber.GetString();
		send.jcxbh = &wstrLineNum;

		strTemp.Format(L"ns1__getAccessTokenca, send.jcxbh:%s", send.jcxbh->c_str());
		CNHLogAPI::WriteLogEx(m_strLogFilePath, L"发送", L"获取联网令牌", strTemp);

		int nRet = toasb.getAccessToken(&send, sRes);

		if (nRet == SOAP_OK)
		{
			std::wstring strRet = sRes.return_->c_str();

			strTemp.Format(L"ns1__getAccessTokenca,sRes.return_:%s", strRet.c_str());
			CNHLogAPI::WriteLogEx(m_strLogFilePath, L"接受", L"获取联网令牌", strTemp);
			...
			if (xmlReader.Parse(strRet.c_str()))
			{
				...
				if (strCode.find(L"1") != std::string::npos)
				{
					...
					if (strAccessToken != L"")
					{
						m_edLincenseCode.SetWindowTextW(strAccessToken.c_str());
						bRet = true;
					}
				}
				else
				{
					strTemp.Format(L"返回错误信息%d", _wtoi(strCode.c_str()));
					CNHLogAPI::WriteLogEx(m_strLogFilePath, L"参数设置", L"获取联网令牌", strTemp);
				}
			}
			else
			{
		...
		{
			strTemp.Format(L"连接平台失败，错误代码为%d", nRet);
			CNHLogAPI::WriteLogEx(m_strLogFilePath, L"参数设置", L"获取联网令牌", strTemp);
		}
	}
...
bool CHNSYDLGDlg::GetTestLog(void)
{
	bool bRet(false);
...
void CHNSYDLGDlg::SetVehWin(const TESTLOG &sTestLog)
{
...
CString CHNSYDLGDlg::ToTestType(const CString& strTestTypeCode)
{
	CString strTestType;
...
CString CHNSYDLGDlg::ToItem(const CString& strItemCode)
{
	CString strItem;
...
bool CHNSYDLGDlg::SaveINIAndSQLAndUP(void)
{
	...
	// 上传部分
	CString strURL, strLineNumber, strLincenseCode, strStationNumber;
	m_edURL.GetWindowTextW(strURL);
	m_edLineNumber.GetWindowTextW(strLineNumber);
	m_edStationNumber.GetWindowTextW(strStationNumber);
	m_edLincenseCode.GetWindowTextW(strLincenseCode);

	if (strURL.IsEmpty() || strLineNumber.IsEmpty() || strLincenseCode.IsEmpty() || strStationNumber.IsEmpty())
	{
	...
	CString strTempMsg;
	strMsg.Empty();
	if (!UpOBDIURP(pchURL, strLineNumber, strLincenseCode, strStationNumber, m_sTestLog, sResultOfOBD, strMsg))
	{
		strMsg = L"上传OBDIURP失败\r\n" + strMsg;
		strTempMsg.AppendFormat(L"%s\r\n", strMsg);
		m_edMsg.SetWindowTextW(strTempMsg);
	}
	if (!UpOBDReaust(pchURL, strLineNumber, strLincenseCode, strStationNumber, m_sTestLog, sResultOfOBD, strMsg))
	{
		strMsg = L"上传OBD结果失败\r\n" + strMsg;
		strTempMsg.AppendFormat(L"%s\r\n", strMsg);
		m_edMsg.SetWindowTextW(strTempMsg);
...
bool CHNSYDLGDlg::SetOBDLOG(SResultOfOBD& sResultOfOBD, CString& strMsg)
{
	bool bRet(true);
...
CString CHNSYDLGDlg::GetOBDType(const int& nType, const CString& strFulType)
{
	if (strFulType == L"汽油")
	{
		switch (nType)
...
void CHNSYDLGDlg::GetEngineCALID(const CString& strOBDType, const CString& strFulType,
...
bool CHNSYDLGDlg::UpOBDIURP(const char * pchURL, const CStringW& strLineNumber, const CStringW& strLincenseCode, const CStringW& strStationNumber, 
...
bool CHNSYDLGDlg::UpOBDReaust(const char * pchURL, const CStringW& strLineNumber, const CStringW& strLincenseCode, const CStringW& strStationNumber, 
	const TESTLOG& sTestLog, const SResultOfOBD& sResultOfOBD, CString& strMsg)
{
	bool bRet(false);
	...
	// 检验流水号
	std::wstring strlsh;
	strlsh = sTestLog.wchRunningNumber;
	send.lsh = &strlsh;
	//clsbm	VIN(车架号)
	std::wstring strVin;
	...
	CString strLog;
	strLog.Format(L"流水号:%s VIN(车架号):%s, OBD执行标准:%s, 车辆行驶里程:%d, 发动机控制单元标定识别码cid:%s, 发动机控制单元校准核查号码:%s, 后处理控制单元标定识别码cid:%s, \
				   ...
				   send.lsh->c_str(), send.clsbm->c_str(), send.obdzsbz->c_str(), send.clxslc, send.fdjkzdycid->c_str(), send.fdjkzdycvn->c_str(), send.hclkzdycid->c_str(), send.hclkzdycvn->c_str(), 
