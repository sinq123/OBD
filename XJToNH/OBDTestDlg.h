...
private:
	...
	bool UpOBDReaust(CString& strMsg);
	bool UPRealTimeData(CStringW& strMsg);
	bool UpOBDIURP(CString& strMsg);
	void SetTestLogAndVehDB(const CString& strItemOBD, const CString& strHasOBD, CString& strstrMsg);
	
	// OBD过程数据
	std::vector<SOBDRTData> m_vtsOBDRTData;
	bool SetRTToSQL(const std::vector<SOBDRTData>& m_vtsOBDRTData, const CString& strTestType);
};
