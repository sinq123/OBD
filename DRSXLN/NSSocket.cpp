...
bool CNSSocket::Open(LPCTSTR szIP/*=NULL*/, const int nPort/*=NULL*/)
{
	...
	{
		CNHLogAPI::WriteLogEx(CNSSocket::LogPath(), L"Open", L"连接成功", strLog);
		return true;
	}
	else
	{
		CNHLogAPI::WriteLogEx(CNSSocket::LogPath(), L"Open", L"连接失败", strLog);
		Close();
		return false;
	}
}
...
bool CNSSocket::Close(void)
{
	if (m_socket != INVALID_SOCKET)
	{
		...
		CNHLogAPI::WriteLogEx(CNSSocket::LogPath(), L"Close", L"关闭连接", L"");
	}
	return true;
}
...
bool CNSSocket::SendAndRecvPacket(LPCTSTR szSend, std::wstring &strRecv)
{
	...
	{
		CNHLogAPI::WriteLogEx(CNSSocket::LogPath(), L"SendAndRecvPacket", L"发送成功", szSend);
		...
		{
			CNHLogAPI::WriteLogEx(CNSSocket::LogPath(), L"SendAndRecvPacket", L"接收成功", strRecv.c_str());
				
			return true;
		}
		else
		{
			CNHLogAPI::WriteLogEx(CNSSocket::LogPath(), L"SendAndRecvPacket", L"接收失败", strRecv.c_str());
...
bool CNSSocket::RecvAll(SOCKET sockfd, char *recvbuf, int len)
{
	...
	{  
		...
		{
			CNHLogAPI::WriteLogEx(CNSSocket::LogPath(), L"RecvAll", L"接收超时", L"");
			return false;
		}
		else if (nRet == 0)
		{
			CNHLogAPI::WriteLogEx(CNSSocket::LogPath(), L"RecvAll", L"服务端已断开", L"");
			Close();
			return false;
		}
		nLeft -= nRet;
...
bool CNSSocket::SendAll(SOCKET sockfd, char *sendbuf, int len)
{
	...
	while (nLeft > 0)
	{
		...
		if (nRet == SOCKET_ERROR)
		{
			CNHLogAPI::WriteLogEx(CNSSocket::LogPath(), L"SendAll", L"服务端已断开", L"");
			Close();
			return false;
		}
		nLeft -= nRet;
...
CStringW CNSSocket::LogPath(void)
{
	// 日志文件所在文件夹路径
	...
	strFileName.Format(L"DRSXLN%s.log", odtNow.Format(L"%Y-%m-%d"));
