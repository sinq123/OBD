...
void CDynDlg::OnBnClickedButtonConstLoad()
{
	// TODO: 在此添加控件通知处理程序代码
	DeleteInfoFile();
	m_otdStartConstLoad = COleDateTime::GetCurrentTime();
...
{
	// TODO: 在此添加控件通知处理程序代码
	m_otdStartPLHP = COleDateTime::GetCurrentTime();
...
DWORD CDynDlg::WriteDynConstLoadData(void)
{
	...
	if (CSocketLib_API::GetInstance().UploadDeviceChkProcess(L"1", g_pDynDlg->m_otdStartConstLoad.Format(L"%Y-%m-%d %H:%M:%S").GetString(), COleDateTime::GetCurrentTime().Format(L"%Y-%m-%d %H:%M:%S").GetString(), strResult, strMsg))
	{
		CString str = si.GetString(L"LowerRangeLoadConst", L"RangeLoadConstType", L"1");
		if (L"3" == str) // LUGDOWN
		{
			CSocketLib_API::GetInstance().UploadDiesDynCoastdown(strResult, strMsg);
		}
		else // 
		{
			CSocketLib_API::GetInstance().UploadGasDynCoastdown(strResult, strMsg);
		}
...
DWORD CDynDlg::WriteDynPLHPData(void)
{
	...
	std::wstring strResult, strMsg;
	if (CSocketLib_API::GetInstance().UploadDeviceChkProcess(L"2", g_pDynDlg->m_otdStartPLHP.Format(L"%Y-%m-%d %H:%M:%S").GetString(), COleDateTime::GetCurrentTime().Format(L"%Y-%m-%d %H:%M:%S").GetString(), strResult, strMsg))
	{
		CString str = si.GetString(L"ResultOfDynPLHP", L"PLHPType", L"0");
		if (L"3" == str) // 柴油
		{
			CSocketLib_API::GetInstance().UploadDiesDynParasiticLose(strResult, strMsg);
		}
		else // 汽油
		{
			CSocketLib_API::GetInstance().UploadGasDynParasiticLose(strResult, strMsg);
		}
