...
CString CNOxDlg::m_strJudgeMent[3] = {L"1", L"1", L"1"};

CNOxDlg* g_pNOxDlg(NULL);
...
void CNOxDlg::OnBnClickedButtonGasChk()
{
	...
	m_otdStart = COleDateTime::GetCurrentTime();
...
DWORD CNOxDlg::WriteAnaGasChkData(void)
{
	...
	{
		m_strJudgeMent[0] = si.GetString(L"ResultOfAnaGasChk", L"SkipCO2Judgement", L"0");
	}
	// NO
	if (m_strJudgeMent[2] == L"1")
	{
		m_strJudgeMent[1] = si.GetString(L"ResultOfAnaGasChk", L"SkipNOJudgement", L"0");
	}
	// NO2
	if (m_strJudgeMent[2] == L"1")
	{
		m_strJudgeMent[2] = si.GetString(L"ResultOfAnaGasChk", L"SkipNO2Judgement", L"0");
	}
	...
	{
		si.SetString(L"ResultOfAnaGasChk", L"SkipCO2Judgement", L"0");
		si.SetString(L"ResultOfAnaGasChk", L"SkipNOJudgement", L"0");
		si.SetString(L"ResultOfAnaGasChk", L"SkipNO2Judgement", L"0");

		std::wstring strResult, strMsg;
		bool bUploadDeviceChk(false);
		if ((str == L"4"))
		{
			// 高标气
			if (CSocketLib_API::GetInstance().UploadAnaHighStdGasCheck(strResult, strMsg))
			{
				if (CSocketLib_API::GetInstance().UploadDeviceChkProcess(L"5", g_pNOxDlg->m_otdStart.Format(L"%Y-%m-%d %H:%M:%S").GetString(), COleDateTime::GetCurrentTime().Format(L"%Y-%m-%d %H:%M:%S").GetString(), strResult, strMsg))
				{
					CSocketLib_API::GetInstance().UploadAnaFullCalChk(strResult, strMsg);
					CSocketLib_API::GetInstance().UploadDeviceChkProcess(L"6", g_pNOxDlg->m_otdStart.Format(L"%Y-%m-%d %H:%M:%S").GetString(), COleDateTime::GetCurrentTime().Format(L"%Y-%m-%d %H:%M:%S").GetString(), strResult, strMsg);
				}
				else
				{
		...
		else if ((str == L"2") || (str == L"3"))
		{
			// 中低浓度按五点上传
			if (CSocketLib_API::GetInstance().UploadDeviceChkProcess(L"6", g_pNOxDlg->m_otdStart.Format(L"%Y-%m-%d %H:%M:%S").GetString(), COleDateTime::GetCurrentTime().Format(L"%Y-%m-%d %H:%M:%S").GetString(), strResult, strMsg))
			{
				CSocketLib_API::GetInstance().UploadAnaFullCalChk(strResult, strMsg);
			}
			else
		...
		{
			// 低标气
			if (CSocketLib_API::GetInstance().UploadAnaLowStdGasCheck(strResult, strMsg))
			{
				if (CSocketLib_API::GetInstance().UploadDeviceChkProcess(L"3", g_pNOxDlg->m_otdStart.Format(L"%Y-%m-%d %H:%M:%S").GetString(), COleDateTime::GetCurrentTime().Format(L"%Y-%m-%d %H:%M:%S").GetString(), strResult, strMsg))
				{
					CSocketLib_API::GetInstance().UploadAnaFullCalChk(strResult, strMsg);
					CSocketLib_API::GetInstance().UploadDeviceChkProcess(L"6", g_pNOxDlg->m_otdStart.Format(L"%Y-%m-%d %H:%M:%S").GetString(), COleDateTime::GetCurrentTime().Format(L"%Y-%m-%d %H:%M:%S").GetString(), strResult, strMsg);
				}
