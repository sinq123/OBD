...
DWORD COpaDlg::WriteOpaCalChkData(void)
{
	...
	//if (CSocketLib_API::GetInstance().UploadDeviceChkProcess(L"1", g_pOpaDlg->m_otdStartFormat(L"%Y-%m-%d %H:%M:%S").GetString(), COleDateTime::GetCurrentTime().Format(L"%Y-%m-%d %H:%M:%S").GetString(), strResult, strMsg))
	//{
	//}

	CSocketLib_API::GetInstance().UploadOpaCalChk(strResult, strMsg);
