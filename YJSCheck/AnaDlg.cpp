...
DWORD CAnaDlg::WriteAnaGasChkData(void)
{
		...
		)
	{
		...
		if ((str == L"4"))
		{
			...
			{
				if (CSocketLib_API::GetInstance().UploadDeviceChkProcess(L"5", g_pAnaDlg->m_otdStart.Format(L"%Y-%m-%d %H:%M:%S").GetString(), COleDateTime::GetCurrentTime().Format(L"%Y-%m-%d %H:%M:%S").GetString(), strResult, strMsg))
				{
					CSocketLib_API::GetInstance().UploadAnaFullCalChk(strResult, strMsg);
					CSocketLib_API::GetInstance().UploadDeviceChkProcess(L"6", g_pAnaDlg->m_otdStart.Format(L"%Y-%m-%d %H:%M:%S").GetString(), COleDateTime::GetCurrentTime().Format(L"%Y-%m-%d %H:%M:%S").GetString(), strResult, strMsg);
				}
				else
				{
		...
		{
			// 中低浓度按五点上传
			if (CSocketLib_API::GetInstance().UploadAnaFullCalChk(strResult, strMsg))
			{
				CSocketLib_API::GetInstance().UploadDeviceChkProcess(L"6", g_pAnaDlg->m_otdStart.Format(L"%Y-%m-%d %H:%M:%S").GetString(), COleDateTime::GetCurrentTime().Format(L"%Y-%m-%d %H:%M:%S").GetString(), strResult, strMsg);
			}
			else
		...
		else
		{
			...
			{
				if (CSocketLib_API::GetInstance().UploadDeviceChkProcess(L"3", g_pAnaDlg->m_otdStart.Format(L"%Y-%m-%d %H:%M:%S").GetString(), COleDateTime::GetCurrentTime().Format(L"%Y-%m-%d %H:%M:%S").GetString(), strResult, strMsg))
				{
					// 五点上传
					CSocketLib_API::GetInstance().UploadAnaFullCalChk(strResult, strMsg);
					CSocketLib_API::GetInstance().UploadDeviceChkProcess(L"6", g_pAnaDlg->m_otdStart.Format(L"%Y-%m-%d %H:%M:%S").GetString(), COleDateTime::GetCurrentTime().Format(L"%Y-%m-%d %H:%M:%S").GetString(), strResult, strMsg);
